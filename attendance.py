...
class Attendace:
    def __init__(self, root):
        ...
        img10 = Image.open(
            r"C:\Users\sumit\Downloads\Facial_Recognition_Automatic_Attendance_System\Main_W_Facial_Recognition_Automatic_Attendance_System_Final_Year_Project\college_images\hqdefault.jpg")
        ...
        img11 = Image.open(
            r"C:\Users\sumit\Downloads\Facial_Recognition_Automatic_Attendance_System\Main_W_Facial_Recognition_Automatic_Attendance_System_Final_Year_Project\college_images\AdobeStock_303989091.jpeg")
        ...
        img1 = Image.open(
            r"C:\Users\sumit\Downloads\Facial_Recognition_Automatic_Attendance_System\Main_W_Facial_Recognition_Automatic_Attendance_System_Final_Year_Project\college_images\un.jpg")
        ...
        title = Label(self.root, text="STUDENT ATTENDANCE MANAGEMENT SYSTEM", font=(
            "times new roman", 35, "bold"), bg="white", fg="crimson")
        ...
        img30 = Image.open(
            r"C:\Users\sumit\Downloads\Facial_Recognition_Automatic_Attendance_System\Main_W_Facial_Recognition_Automatic_Attendance_System_Final_Year_Project\college_images\face-recognition.png")
        img30 = img30.resize((650, 150), Image.ANTIALIAS)
        self.photoImg30 = ImageTk.PhotoImage(img30)
        bg_lbl = Label(DataFrameLeft, image=self.photoImg30,
                       bd=2, relief=RIDGE)
        bg_lbl.place(x=0, y=0, width=650, height=150)

        empLabel = Label(updateframe, text="Student ID:",
                         bg="white", font="comicsansns 11 bold")
        empLabel.grid(row=0, column=0, padx=4, pady=8)

        nameLabel = Label(updateframe, text="Roll:",
                          bg="white", font="comicsansns 11 bold")
        nameLabel.grid(row=0, column=2, padx=4, pady=8)

        dateLabel = Label(updateframe, text="Name:",
                          bg="white", font="comicsansns 11 bold")
        dateLabel.grid(row=1, column=0)

        dateLabel = Label(updateframe, text="Department:",
                          bg="white", font="comicsansns 11 bold")
        dateLabel.grid(row=1, column=2)

        dateLabel = Label(updateframe, text="Time:",
                          bg="white", font="comicsansns 11 bold")
        dateLabel.grid(row=2, column=0)

        dateLabel = Label(updateframe, text="Date:",
                          bg="white", font="comicsansns 11 bold")
        dateLabel.grid(row=2, column=2)

        attendanceLabel = Label(
            updateframe, text="Attendance Status", bg="white", font="comicsansns 11 bold")
        attendanceLabel.grid(row=3, column=0)

        # ============================= Entry ====================================================
        self.atten_id = ttk.Entry(
            updateframe, textvariable=self.var_atten_id, width=22, font="comicsansns 11 bold")
        self.atten_id.grid(row=0, column=1, pady=8)
        self.atten_id.focus()

        atten_name = ttk.Entry(
            updateframe, textvariable=self.var_atten_roll, width=22, font="comicsansns 11 bold")
        atten_name.grid(row=0, column=3, pady=8)

        atten_date = ttk.Entry(
            updateframe, textvariable=self.var_atten_name, width=22, font="comicsansns 11 bold")
        atten_date.grid(row=1, column=1, pady=8)

        atten_date = ttk.Entry(
            updateframe, textvariable=self.var_atten_dep, width=22, font="comicsansns 11 bold")
        atten_date.grid(row=1, column=3, pady=8)

        atten_date = ttk.Entry(
            updateframe, textvariable=self.var_atten_time, width=22, font="comicsansns 11 bold")
        atten_date.grid(row=2, column=1, pady=8)

        atten_date = ttk.Entry(
            updateframe, textvariable=self.var_atten_date, width=22, font="comicsansns 11 bold")
        atten_date.grid(row=2, column=3, pady=8)

        self.atten_status = ttk.Combobox(
            updateframe, width=22, textvariable=self.var_atten_attendance, font="comicsansns 11 bold", state="readonly")
        ...
        btnAddData = Button(ButtonFrame1, text="Import csv", command=self.importData, font=(
            "arial", 11, "bold"), width=16, bg="lime", fg="white")
        btnAddData.grid(row=0, column=0, padx=1)

        btnUpdate = Button(ButtonFrame1, text="Export csv", command=self.export_data, font=(
            "arial", 11, "bold"), width=16, bg="lime", fg="white")
        btnUpdate.grid(row=0, column=1, padx=1)

        btnDelete = Button(ButtonFrame1, text="Update", command=self.action, font=(
            "arial", 11, "bold"), width=16, bg="lime", fg="white")
        btnDelete.grid(row=0, column=2, padx=1)

        btnReset = Button(ButtonFrame1, text="Reset", command=self.clear, font=(
            "arial", 11, "bold"), width=16, bg="lime", fg="white")
        ...
        self.AttendanceReportTable = ttk.Treeview(AttendanceFrame, column=(
    ...
    # ======back=========
        btnback = Button(title, text="Back", command=self.go_back, font=(
        ...
        try:
            if len(mydata) < 1:
                messagebox.showerror(
                    "No Data", "No Data to export", parent=self.root)
                return False
            fln = filedialog.asksaveasfilename(initialdir=os.getcwd(), title="Save CSV", filetypes=(
                ("CSV file", "*.csv"), ("All File", "*.*")), parent=self.root)
            with open(fln, mode="w", newline="") as myfile:
                exp_writer = csv.writer(myfile, delimiter=",")
                for i in mydata:
                    exp_writer.writerow(i)
                messagebox.showinfo("Data Exported", "Your data exported to " +
    ...
    def action(self):
        ...
        try:
            ...
            with open(fln, mode="a", newline="\n") as f:
                dict_writer = DictWriter(f, fieldnames=(
