...
class ChatBot:
    def __init__(self, root):
        ...
        Title_label = Label(main_frame, bd=3, relief=RAISED, anchor='nw', width=730, image=self.photoimg_chat,
                            compound=LEFT, text="CHAT ME", font=("arial", 30, 'bold'), fg='green', bg="white")
        # Title_label.grid(row=0,column=0,sticky=W)
        Title_label.pack(side=TOP)

        self.scroll_y = ttk.Scrollbar(main_frame, orient=VERTICAL)
        self.text = Text(main_frame, yscrollcommand=self.scroll_y.set,
                         width=65, height=20, bd=3, relief=RAISED, font=("arial", 14))
        # self.text.grid(row=1,column=0,padx=2,sticky=W)
        self.scroll_y.pack(side=RIGHT, fill=Y)
        self.text.pack()

        btn_frame = Frame(self.root, bd=3, relief=RAISED,
                          bg='white', width=730)
        btn_frame.pack()

        label = Label(btn_frame, text="Type Something",
                      fg='green', bg='white', font=("arial", 14))
        label.grid(row=0, column=0, padx=5, sticky=W)

        self.entry = StringVar()
        self.e = ttk.Entry(btn_frame, textvariable=self.entry,
                           width=40, font=("times new roman", 17))
        self.e.grid(row=0, column=1, padx=5, sticky="w")
        self.e.focus()

        self.send = Button(btn_frame, text="Send->>", command=self.send,
                           width=8, bg="green", fg="white", font=("times new roman", 16))
        self.send.grid(row=0, column=2, sticky=W)

        self.clear = Button(btn_frame, text="Clear Chat ", command=self.clear,
                            width=8, bg="red", fg="white", font=("times new roman", 15))
        self.clear.grid(row=1, column=0, sticky=W)

        self.msg = ""
        self.label1 = Label(btn_frame, text=self.msg,
